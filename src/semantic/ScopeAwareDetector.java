...
public abstract class ScopeAwareDetector implements ErrorDetector {
    ...
    protected String getEnclosingFunctionName() {
        // Subclasses that need function name tracking should maintain their own
        // function stack (see InfiniteRecursionDetector for example).
        // This default implementation requires function names to be registered
        // in the scope tree (via enterFunction hook), which is done by
        // UndefinedSymbolDetector but not by other detectors.
        Scope s = currentScope;
        while (s != null) {
    ...
    protected void walkFunction(FunctionDefinition fd) {
        enterFunction(fd);
        // Subclasses that need function name / params registered in the scope
        // should do so in enterFunction() via defineSymbol().
        // The base class does NOT register them automatically to avoid
        // each detector producing duplicate errors for the same declaration.
    ...
    /**
     * Hook called BEFORE pushScope. Subclasses that need to register the function
     * name in the enclosing scope override this and call defineSymbol.
     * Also the right place to register parameters (though params are also registered
     * automatically after pushScope — override enterFunction to control this).
     */
    ...
    /**
     * Hook called BEFORE pushScope. Subclasses that need the loop variable
     * registered in the for-loop scope override this and call defineSymbol.
     */
